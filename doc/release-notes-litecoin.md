Runencoin Core version *0.15.1* is now available from:

  <https://download.runencoin.org/runencoin-0.15.1/>
...
A number of changes to the way Runencoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 tors per kB (0.00001 RNC). This is to prepare for dropping the minimum transaction fee to 0.1 tors per kB (0.0001 RNC) in the next release.
