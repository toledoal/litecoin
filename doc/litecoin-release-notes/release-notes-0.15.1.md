Moneda Core version *0.15.1* is now available from:

  <https://download.moneda.org/moneda-0.15.1/>
...
A number of changes to the way Moneda Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 pesos per kB (0.00001 MND). This is to prepare for dropping the minimum transaction fee to 0.1 pesos per kB (0.0001 MND) in the next release.
