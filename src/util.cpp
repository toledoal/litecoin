...
const char * const BITCOIN_CONF_FILENAME = "runencoin.conf";
const char * const BITCOIN_PID_FILENAME = "runencoin.pid";
...
#else
    const char* pszModule = "runencoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Runencoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Runencoin";
#else
    // Unix
    return pathRet / ".runencoin";
#endif
#endif
}
